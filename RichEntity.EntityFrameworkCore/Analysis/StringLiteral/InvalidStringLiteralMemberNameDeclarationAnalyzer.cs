...
using RichEntity.EntityFrameworkCore.Analysis.Extensions;
using RichEntity.EntityFrameworkCore.Analysis.LiteralNameInvocationLocators.Base;
using RichEntity.Extensions;

namespace RichEntity.EntityFrameworkCore.Analysis.StringLiteral;
...
public sealed class InvalidStringLiteralMemberNameDeclarationAnalyzer : DiagnosticAnalyzer
{
    ...
    private void AnalyzeOperation(OperationAnalysisContext context)
    {
        ...
        if (genericType is not INamedTypeSymbol namedTypeSymbol)
            return;

        // Type is imported from metadata and fields are not available.
        if (namedTypeSymbol.Locations.All(l => l.IsInMetadata) &&
            context.Compilation.Options.MetadataImportOptions is not MetadataImportOptions.All)
            return;
